...
GameObject::GameObject(Guid guid) : _isActive{ true }, _guid{ guid }
{
	_transform = std::make_unique<Transform>(glm::vec3{0.0f,0.0f, 0.0f}, glm::vec2{ 0.0f,0.0f }, glm::vec2{ 1.0f, 1.0f });
	_spriteRenderer = std::make_unique<SpriteRenderer>(_transform.get(), _shader);
}

GameObject::GameObject(Guid guid, glm::vec3 position, glm::vec2 rotation, glm::vec2 scale) : _isActive{ true }, _guid{ guid }
{
